...
class PartnerSelectInput extends Component {
    constructor(props) {
        super(props);
		...
		for (var available_id in available_list) {
			...
			items.push(
				<option
					...
					{available_option.first_name + " " + available_option.last_name}
				</option>);
	...
	onDropdownSelected(event, available_list, selected_list) {
		...
		if(Object.keys(selected_list).indexOf(selected_id) <= -1) {
			...
			var selected_name = selected_option.first_name + " " + selected_option.last_name;
